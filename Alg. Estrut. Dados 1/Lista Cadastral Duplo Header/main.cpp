...
//OPERACOES DIFERENTES DA F5 ----- 

//Numero de itens que estao em uma lista original e nao estao numa lista de "Referencia"- Spider shopping
int itensErrados(ListaCadastral &original, ListaCadastral &referencia){
    int errados = 0;
    int i;
    //Percorrendo a lista original
    i = original.pegaOPrimeiro();
    while(i != NULO){
        if(!referencia.estaNaLista(i)) errados++;
        i = original.pegaOProximo(i);
    }

    return errados;
}

//Numero de itens diferentes em geral entre duas listasa
int itensDiferentes(ListaCadastral &x, ListaCadastral &y){
    int diferentes = 0;
    int i;
    //Lista auxiliar para guardar elementos que já foram percorridos
    ListaCadastral temp;
    ...
    while(i!=NULO){
        if(!x.estaNaLista(i) && !temp.estaNaLista(i)) diferentes++;
        i = y.pegaOProximo(i);
    }
...
int main(){
    ...
    ////////////////////////////////
    cout << "Testando agora op. não primitivas da F6" << endl;
    
    ListaCadastral lista5, lista6;

    for(int i=0; i<10; i++) lista5.insere(i);
    for(int i=5; i<13; i++) lista6.insere(i);

    cout << "Tendo como referencia a lista 5, a lista 6 tem " << itensErrados(lista6, lista5) << " itens errados." << endl;
    cout << "As listas 5 e 6 possuem " << itensDiferentes(lista5, lista6) << " itens diferentes" << endl;
